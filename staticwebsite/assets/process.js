const API_URL = "https://byddpvhe3b.execute-api.us-east-1.amazonaws.com/dev";
const PHOTOGALLERY_S3_BUCKET_URL = "photobucket-gupta-2024-4150";
...
function deletePhoto(photoid) {
    var dataDir = {
        "body-json": { "photoID": photoid }
    };
    console.log(dataDir); 
    $.ajax({
        url:  `${API_URL}/photos`, // Replace API_URL with the actual URL
        type: 'DELETE',
        crossDomain: true,
        dataType: 'json',
        contentType: 'application/json',
...
function loadViewPhotoPage(){
    ...
    $.ajax({
        ...
        success: function(data) {            
            console.log(data);
            photo=data.body[0];
            htmlstr = htmlstr + '<img class=\"img-responsive\" src=\"'+photo.URL+'\" alt=\"\"> <div style="display: block;position: relative;float: right;margin: 1em;"><button id="photo_update" onclick="showUpdateForm(this)" data-photo="' + photo.PhotoID + '">Update</button> <button id="photo_delete" data-photo="' + photo.PhotoID + '">Delete</button></div><div class=\"blog-grid-content\"> <h2 class=\"blog-grid-title-lg\"><a class=\"blog-grid-title-link\" href=\"#\">'+photo.Title+'</a></h2> <p>By: '+photo.Username+'</p> <p>Uploaded: '+photo.CreationTime+'</p> <p>'+photo.Description+'</p></div>'
...
$('#portfolio-4-col-grid').on('click', '#home_delete', function(e) {
    ...
    // Pass the photo ID and creation time to the deletePhoto function
...
$('#viewphoto-container').on('click', '#photo_update', function(e) {
    ...
    var newTags = prompt("Enter new tags (comma-separated):");

    // Check if the user pressed "Cancel" in any prompt
    if (newTitle === null || newDescription === null || newTags === null) {
        console.log("Update canceled");
        return;
    }

    // Get values from the button data
    var photoID = $this.data('photo');

    // Call the updatePhotoDetails function
    updatePhotoDetails(photoID, newTitle, newDescription, newTags);
});

function updatePhotoDetails(photoID, title, description, tags) {
    // Call your API Gateway or Lambda function to update photo details
    $.ajax({
        url: `${API_URL}/photos/${photoID}`,  // Replace with your API Gateway URL
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({
            photoID: photoID,
        ...
        error: function(error) {
            ...
            alert("Error updating photo details. Please try again.");
        }
    });
}
